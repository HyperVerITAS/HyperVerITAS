...
fn main() {
    ...
    println!("Full System Gray Brake");
    ...
    for iii in 0..1 {
        ...
        let imgPolyFriCom = Pcs::batch_commit_and_write(&pp, &imgPoliesFri, &mut transcriptFri).unwrap();
        ...
        for i in 0..3{
            let mut valsH: Vec<FriFR> = hRGB[i]
                .evaluations
                .iter()
                .map(|x| kzgFieldToFriField(x))
                .collect();
            ...
            let valsProdX: Vec<FriFR> = prod_xRGB[i]
                .evaluations
                .iter()
                .map(|x| kzgFieldToFriField(x))
                .collect();
            // println!("{:?}", imgValsH.len());
            // let polyProdFri = MultilinearPolynomial::<FriFR>::from_evals(valsProdX);
            // friPolies.push(&polyProdFri);

            let valsFracX: Vec<FriFR> = frac_xRGB[i]
                .evaluations
                .iter()
                .map(|x| kzgFieldToFriField(x))
                .collect();
            // println!("{:?}", imgValsH.len());
            // let FracFri = MultilinearPolynomial::<FriFR>::from_evals(valsFracX);
            // friPolies.push(&FracFri);
            friPolyVectors.push(valsH);
            friPolyVectors.push(valsProdX);
            friPolyVectors.push(valsFracX);
        }
        // multsetProofGray, fxGray, gxGray, hGray

        // ==================================================================POLY STUFF FOR GRAYSCALE==================================================================
        let mut valsGrayErrPolyFri: Vec<FriFR> = grayErrPoly
            .evaluations
            .iter()
            .map(|x| kzgFieldToFriField(x))
            .collect();
        for j in 0..1<<numCols{
                valsGrayErrPolyFri.push(FriFR::ZERO);
            }

        let mut valsH: Vec<FriFR> = hGray
            .evaluations
            .iter()
            .map(|x| kzgFieldToFriField(x))
            .collect();
        ...
        let valsProdX: Vec<FriFR> = gxGray
            .evaluations
            .iter()
            .map(|x| kzgFieldToFriField(x))
            .collect();
        ...
        let valsFracX: Vec<FriFR> = fxGray
            .evaluations
            .iter()
            .map(|x| kzgFieldToFriField(x))
            .collect();
        ...
        // let FracFri = MultilinearPolynomial::<FriFR>::from_evals(valsFracX);
        // friPolies.push(&FracFri);
        friPolyVectors.push(valsGrayErrPolyFri);

        
        friPolyVectors.push(valsH);
        friPolyVectors.push(valsProdX);
        friPolyVectors.push(valsFracX);
            // ==================================================================END POLY STUFF FOR GRAYSCALE==================================================================

        friPolies = friPolyVectors.iter()
        .map(|x| MultilinearPolynomial::<FriFR>::from_evals(x.clone()))
        .collect();
        let friPolyComs = Pcs::batch_commit_and_write(&pp, &friPolies, &mut transcriptFri);
        // let elapsed_time = nowOpens.elapsed();
        ...
        // Point to prove matrix-hash mult was correct
        for i in 0..3{
            let mut origPt: Vec<FriFR> = proofRGB[i].point.iter().map(|x| kzgFieldToFriField(x)).collect();
        ...
        // This is random point we open for image transformation. We'll make this random for grayscaling
        // ==========================================Grayscale Points==========================================
        //Point to prove error and orig img are correct
        ...
        // //----------------------------------------------We first add opening for matrixMult for hash.----------------------------------------------    
            ...
            evals.push(Evaluation::new(
                ...
                polynomials[3+ i* 3].evaluate(&points[3]),
            ));
            ...
            evals.push(Evaluation::new(
                ...
                polynomials[i].evaluate(&points[5 + i*6]),
            ));
            ...
            evals.push(Evaluation::new(
                ...
                polynomials[3+i*3].evaluate(&points[6 + i*6]),
            ));
            ...
            evals.push(Evaluation::new(
                ...
                polynomials[3+i*3].evaluate(&points[7 + i*6]),
            ));
            ...
            evals.push(Evaluation::new(
                ...
                polynomials[4+i*3].evaluate(&points[4]),
            ));
            ...
            evals.push(Evaluation::new(
                4+i*3, //This is the poly index
                9+i*6, //This is the point index
                polynomials[4+i*3].evaluate(&points[9+i*6]),
            ));
            ...
            evals.push(Evaluation::new(
                5+i*3, //This is the poly index
                9+i*6, //This is the point index
                polynomials[5+i*3].evaluate(&points[9+i*6]),
            ));            
            ...
            evals.push(Evaluation::new(
                4+i*3, //This is the poly index
                10+i*6, //This is the point index
                polynomials[4+i*3].evaluate(&points[10+i*6]),
            ));
            ...
            evals.push(Evaluation::new(
                5+i*3, //This is the poly index
                10+i*6, //This is the point index
                polynomials[5+i*3].evaluate(&points[10+i*6]),
            ));
            // // // //----------------------------------------------GRAYSCALE:we now add transPoint for IMG ----------------------------------------------
            evals.push(Evaluation::new(
                i, //This is the poly index
                23, //This is the point index
                polynomials[i].evaluate(&points[23]),
            ));
        ...
        // println!("{}",3+ i* 3);
        evals.push(Evaluation::new(
            13, //This is the poly index
            3, //This is the point index
            polynomials[13].evaluate(&points[3]),
        ));

        // // //----------------------------------------------GRAYSCALE:We now add alpha_range for grayERRORPoly----------------------------------------------
        evals.push(Evaluation::new(
            12, //This is the poly index
            24, //This is the point index
            polynomials[12].evaluate(&points[24]),
        ));
        // // //----------------------------------------------GRAYSCALE:We now add alpha_range for h----------------------------------------------
        evals.push(Evaluation::new(
            13, //This is the poly index
            24, //This is the point index
            polynomials[13].evaluate(&points[24]),
        ));
    
        // // //----------------------------------------------GRAYSCALE:We now add alpha_range modified0 for h----------------------------------------------
        evals.push(Evaluation::new(
            13, //This is the poly index
            25, //This is the point index
            polynomials[13].evaluate(&points[25]),
        ));

        // // //----------------------------------------------GRAYSCALE:We now add alpha_range modified1 for h----------------------------------------------
        evals.push(Evaluation::new(
            13, //This is the poly index
            26, //This is the point index
            polynomials[13].evaluate(&points[26]),
        ));
        
        // // //----------------------------------------------GRAYSCALE:We then add prod for 1...10----------------------------------------------
        evals.push(Evaluation::new(
            14, //This is the poly index
            4, //This is the point index
            polynomials[14].evaluate(&points[4]),
        ));
    
        // // //----------------------------------------------GRAYSCALE:We now add alpha_range for prod----------------------------------------------
        evals.push(Evaluation::new(
            14, //This is the poly index
            27, //This is the point index
            polynomials[14].evaluate(&points[27]),
        ));

        // // //----------------------------------------------GRAYSCALE:We now add alpha_range for frac----------------------------------------------

        evals.push(Evaluation::new(
            15, //This is the poly index
            27, //This is the point index
            polynomials[15].evaluate(&points[27]),
        ));
        // //----------------------------------------------GRAYSCALE:we now add alpha_range||0 for prod----------------------------------------------

        evals.push(Evaluation::new(
            14, //This is the poly index
            28, //This is the point index
            polynomials[14].evaluate(&points[28]),
        ));
        ...
        evals.push(Evaluation::new(
            15, //This is the poly index
            28, //This is the point index
            polynomials[15].evaluate(&points[28]),
        ));            
        ...
        evals.push(Evaluation::new(
            14, //This is the poly index
            29, //This is the point index
            polynomials[14].evaluate(&points[29]),
        ));
        ...
        evals.push(Evaluation::new(
            15, //This is the poly index
            29, //This is the point index
            polynomials[15].evaluate(&points[29]),
        ));
        // // // //----------------------------------------------GRAYSCALE:we now add transpoint for grayERRORPoly----------------------------------------------

        evals.push(Evaluation::new(
            12, //This is the poly index
            23, //This is the point index
            polynomials[12].evaluate(&points[23]),
        ));
    ...
    let myProof = transcriptFri.into_proof();
    ...
    // let now = Instant::now();
    // // let mut transcriptFri = Keccak256Transcript::new(());
    // let mut transcript = Keccak256Transcript::from_proof((), myProof.as_slice());
