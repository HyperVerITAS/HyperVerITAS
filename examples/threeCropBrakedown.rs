...
fn main() {
    println!("Three Channel Crop Brake");

    const testSize: usize = 25;
    ...
    for iii in 0..1 {
        ...
        let origImg = load_image(&fileName);
        let BLANK = vec_to_poly([F::zero();1<<testSize].to_vec()).0;
        ...
        // println!("Made it");
        let transProofs = cropProveAffineIOP::<F>(numCols,cropNumRows, origImgRPoly, origImgGPoly,origImgBPoly, cropImgPolyR, cropImgPolyG, cropImgPolyB, origImg.cols, origImg.rows,
        ...
        // //----------------------------------------------We first add opening for matrixMult for hash.----------------------------------------------    
            ...
            evals.push(Evaluation::new(
                ...
                i, //This is the point index
                polynomials[i].evaluate(&points[i]),
            ));
        }
