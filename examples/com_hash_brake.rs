...
fn run_hash_com_brake(testSize: usize) {
    ...
    imgPolies.push(MultilinearPolynomial::<F>::new(RGBEvalsFri[0].clone()));
    imgPolies.push(MultilinearPolynomial::<F>::new(RGBEvalsFri[1].clone()));
    imgPolies.push(MultilinearPolynomial::<F>::new(RGBEvalsFri[2].clone()));

    let now2 = Instant::now();
    let imgComs = Pcs::batch_commit_and_write(&pp, &imgPolies, &mut transcriptFri);
...
fn main(){
    ...
    for i in first_size..last_size+1 {
        println!("Hash Commitment Brakedown. Size: 2^{:?}\n", i);
        let _res = run_hash_com_brake(i);
        println!("-----------------------------------------------------------------------");
    }
}