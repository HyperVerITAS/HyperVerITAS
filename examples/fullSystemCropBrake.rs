...
fn main() {
    ...
    println!("Full System Crop Brake");

    // TEST POLYNOMIAL
    // let mut origFieldVec = Vec::new();
    // for i in 0..1<<12{
    //     origFieldVec.push(F::rand(&mut rng));
    // }
    // let testVec: Vec<FriFR> = origFieldVec.iter().map(|x| kzgFieldToFriField(x)).collect();

    // let mut origFieldPt = Vec::new();
    // for i in 0..12{
    //     origFieldPt.push(F::rand(&mut rng));
    // }
    // let testPt: Vec<FriFR> = origFieldPt.iter().map(|x| kzgFieldToFriField(x)).collect();

    // let myTestPoly = MultilinearPolynomial::<FriFR>::from_evals(testVec);
    // let myTestCom = Pcs::commit(&pp,&myTestPoly);
    // let myPt = myTestPoly.evaluate(&testPt);
    ...
    for iii in 0..1 {
        ...
        for i in 0..3{
            let mut origPt: Vec<FriFR> = transProofs[i].point.iter().map(|x| kzgFieldToFriField(x)).collect();
            // println!("{:?}", proofR.point);
            origPt.push(FriFR::ZERO);
            points.push(origPt.clone());
        }
        ...
        // //----------------------------------------------We first add opening for matrixMult for hash.----------------------------------------------    
            ...
            evals.push(Evaluation::new(
                i, //This is the poly index
                23+i, //This is the point index
                polynomials[i].evaluate(&points[23+i]),
            ));
        }
