...
static mySize : usize = 22;
static D : usize = 1<<mySize;
...
//     let file = read_photo("Veri", postfix);
...
fn eval_polynomials(domain: GeneralEvaluationDomain::<Fr>, start: u128, instance_hash: &str, time_ck: &CommitterKey::<Bls12_381>,  postfix: &str)  
...
fn get_filename(prefix: &str, postfix: &str) -> String {
    ...
    // let file = read_photo("Veri", postfix);
fn read_photo(prefix: &str,  postfix: &str) -> BufReader<File> {
    let file = File::open(get_filename(prefix, postfix)).expect("Unable to open file");
...
// ===================================================================================================================================
    ...
    // Timing setup
    let myThreadR: thread::JoinHandle<()> = thread::spawn( || {
    ...
    let filename = get_filename("Veri", "R");
...
// ===================================================================================================================================
    ...
    let (coms0, coms1, coms2, evals0, evals1, evals2, prover_hash_coeffs, proof0, proof1, proof2) = eval_polynomials(domain, start, instance_hash.clone(), &time_ck, "R");
    ...
    let myThreadG: thread::JoinHandle<()> = thread::spawn( || {
    let postFix = "G";
    let start = SystemTime::now();
    let start_epoch = start
        .duration_since(UNIX_EPOCH)
    ...
    // Hash edited photo (instance)
    let filename = get_filename("Veri", postFix);
    let input = Path::new(&filename);
    let binding = try_digest(input).unwrap();
    let instance_hash = binding.as_str();
...
// ===================================================================================================================================
    ...
    // PROVER WORK
    // Create commitments, opening proofs, and claimed evaluations
    let (coms0, coms1, coms2, evals0, evals1, evals2, prover_hash_coeffs, proof0, proof1, proof2) = eval_polynomials(domain, start, instance_hash.clone(), &time_ck, postFix);
    // Create Groth16 proof to verify short_hash=PoseidonHash(full_hash) and linear_combo_hash = hash_coeffs \dot full_hash

    last = print_time_since(start, last, "PROVER TOOK");     
    });
    let myThreadB: thread::JoinHandle<()> = thread::spawn( || {
    let postFix = "B";
    let start = SystemTime::now();
    let start_epoch = start
        .duration_since(UNIX_EPOCH)
...
// ===================================================================================================================================
    ...
    let myThreadCrop: thread::JoinHandle<()> = thread::spawn( || {
        cropThreadKZG::cropSystem(mySize);
    });

    myThreadR.join().unwrap();
    myThreadG.join().unwrap();
    myThreadB.join().unwrap();
    myThreadCrop.join().unwrap();

    let elapsed_time = now.elapsed();
    println!("MULTITHREADED TAKES {:?} seconds", elapsed_time.as_millis() as f64 / 1000 as f64);
// ===================================================================================================================================
