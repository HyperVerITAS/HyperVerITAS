...
-> (FriOpenings<F, D>, [MerkleCap<GoldilocksField, PoseidonHash>; 3], FriProof<F, <PoseidonGoldilocksConfig as GenericConfig<D>>::Hasher, D>, u128)  {
    ...
    let file = read_photo("Veri", postfix, size);
...
fn run_full_crop(size: &usize) {
    ...
    let size_1 = size+0;
    let myThreadR = thread::spawn( move || {
        ...
        prove(start, start, 2, 4, omegaR, &fft_root_tableR, "R", &size_1, pixels, degree);

        println!("R Thread Done");
    });

    let size_2 = size+0;
    let myThreadG = thread::spawn( move || {
        ...
        let start_epoch = start
            ...
            .expect("Time went backwards");
        let start = start_epoch.as_millis();
        prove(start, start, 2, 4, omegaR, &fft_root_tableR, "G", &size_2, pixels, degree);

        println!("G Thread Done");
    });

    let size_3 = size+0;
    let myThreadB = thread::spawn( move || {
        ...
        let start_epoch = start
            ...
            .expect("Time went backwards");
        let start = start_epoch.as_millis();
        prove(start, start, 2, 4, omegaR, &fft_root_tableR, "B", &size_3, pixels, degree);

        println!("B Thread Done");
    });

    let size_4 = size+0;
    let myThreadCropR = thread::spawn( move ||{
        println!("R Crop Thread Running");
        oneCrop::cropSystem(&size_4, "R");
        println!("R Crop Thread Done");
    });

    let size_5 = size+0;
    let myThreadCropG = thread::spawn( move ||{
        println!("G Crop Thread Running");
        oneCrop::cropSystem(&size_5, "G");
        println!("G Crop Thread Done");
    });

    let size_6 = size+0;
    let myThreadCropB = thread::spawn( move ||{
        println!("B Crop Thread Running");
        oneCrop::cropSystem(&size_6, "B");
    ...
    myThreadCropR.join().unwrap();
    myThreadCropG.join().unwrap();
    myThreadCropB.join().unwrap();

    let elapsed_time = now.elapsed();
    println!("Whole System Takes {:?} seconds", elapsed_time.as_millis() as f64 / 1000 as f64);
    println!("-----------------------------------------------------------------------");
}
